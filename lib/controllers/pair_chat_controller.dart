...
/*
- Establishes a real time subscription to listen for changes in specific Appwrite 
  database collection.
- Processes these changes to update the pairing information and initiate chat functionalities.
 */
class PairChatController extends GetxController {
  ...
  void getRealtimeStream() {
    //extracts user's unique ID.
    String uid = Get.find<AuthStateController>().uid!;

    //Constructs the channel string based on database IDs and collection names.
    String channel =
        'databases.$masterDatabaseId.collections.$activePairsCollectionId.documents';

    //uses [realtime.subscribe] to subscribe to the constructed channel.
    subscription = realtime.subscribe([channel]);

    //Listens to the stream of data changes from the subscription.
      ...
      if (data.payload.isNotEmpty) {
        ...
        if (uid1 == uid || uid2 == uid) {
          ...
          switch (action) {
            ...
            case 'delete':
              {
                //closes the real-time subscription.
                subscription?.close;
